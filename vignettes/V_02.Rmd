...
## Ingest data
The tools in `youthvars` extend classes and methods from the [ready4use](https://ready4-dev.github.io/ready4use/) package. Our starting point is `x`, an object containing a `Ready4useDyad` (a dataset and data dictionary pair) that we can download from a remote repository.
...
```{r}
x <- YouthvarsSeries(a_Ready4useDyad = procure(procureSlot(x, 
                                                           "b_Ready4useIngest"),
