...
print_descv_stats_tbl <- function (df, bl_fup_vals_chr = c("Baseline", "Follow-up"), caption_1L_chr = NULL, 
    header_col_nms_chr = NULL, mkdn_tbl_ref_1L_chr = NULL, output_type_1L_chr, 
    round_var_nm_1L_chr, test_1L_lgl = F, variable_nms_chr) 
{
    ...
    if (output_type_1L_chr %in% c("PDF", "HTML")) {
        names_chr <- c("", "", "(N =", paste0(n_inc_1L_dbl, ifelse(output_type_1L_chr == 
            "HTML", "\\)", ")")), "(N =", paste0(n_fup_1L_dbl, 
            ifelse(output_type_1L_chr == "HTML", "\\)", ")")))
        header_chr <- c(" ", " ", 2, 2) %>% stats::setNames(c(c("", 
            ""), header_col_nms_chr))
        if (test_1L_lgl) {
